...
include_once get_lib("sqlmap.SQLMapClient");
include get_lib("sqlmap.SQLMapQueryHandler");
include get_lib("sqlmap.SQLMapResultHandler");
include_once get_lib("sqlmap.SQLMapIncludesHandler");
include get_lib("sqlmap.ibatis.IBatisClientCache");
include get_lib("sqlmap.ibatis.exception.IBatisException");
...
class IBatisClient extends SQLMapClient {
	...
	public function loadXML($obj_path, $external_vars = false) {
		...
		if ($this->getSQLMapClientCache()->cachedXMLElmExists($cache_path)) {
			...
			$xml_schema_file_path = get_lib("xmlfile.schema.ibatis_sql_mapping", "xsd");
