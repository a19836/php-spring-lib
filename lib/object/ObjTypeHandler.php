...
include_once get_lib("object.db.DBPrimitive");
include_once get_lib("object.php.Primitive");
...
class ObjTypeHandler {
	...
	public static function getPHPTypesPaths() {
		...
		foreach ($primitive_types as $type_id => $type_title)
			$types[ "object.php.Primitive($type_id)" ] = $type_title;
		
		$files = array_diff(scandir(__DIR__ . "/php/"), array('.', '..', 'Primitive.php'));
		
		foreach ($files as $file) {
			$file_name = pathinfo($file, PATHINFO_FILENAME);
			$types[ "object.php.$file_name" ] = $file_name;
		} 
	...
	public static function getDBTypesPaths() {
		...
		foreach ($db_types as $type_id => $type_title)
			$types[ "object.db.DBPrimitive($type_id)" ] = $type_title;
	...
	public static function convertSimpleTypeIntoCompositeType($type, $primitive_type = "object.php.Primitive") {
		$type = trim($type);
